...
start(){
	...
	msg -azu 'SELECCIONA UN PERTO'
    while [[ -z "${chekuser}" ]]; do
    	dport=$(shuf -i 82-90 -n 1)
    	chekuser=$(in_opcion -ama "Ingresa un puerto [def = ${dport}]")
    	...
    	if [[ ! $chekuser =~ $numero ]]; then
    		...
    		msg -verm2 'ingresa un numero mayor a 10'
    		sleep 2
    		del 1
    		unset chekuser
    	elif [[ $chekuser -gt 65535 ]]; then
    	msg -verm2 'ingresa un numero menor a 65535'
    		sleep 2
    		del 1
    		unset chekuser
    	elif [[ $(mportas|grep -w "$PORT") ]]; then
			msg -verm2 "Puerto en uso!"
			sleep 2
			del 1
    		unset chekuser
    	fi
